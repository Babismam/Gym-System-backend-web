...
public class AuthResource {
    ...
    public Response register(RegisterDTO dto) {
        ...
        // --- ΜΕΤΑΤΡΟΠΗ: String (DTO) -> LocalDate (Java) ---
        LocalDate dob = null;
        if (dto.getDateOfBirth() != null && !dto.getDateOfBirth().isEmpty()) {
            try {
                dob = LocalDate.parse(dto.getDateOfBirth());
            } catch (Exception e) {
                return Response.status(Response.Status.BAD_REQUEST).entity(Map.of("error", "Μη έγκυρη μορφή ημερομηνίας.")).build();
            }
        }
        ...
        // Χρησιμοποιούμε το 'dob' (LocalDate) που φτιάξαμε, όχι το String του DTO
